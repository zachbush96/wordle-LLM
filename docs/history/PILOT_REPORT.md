...
The experiment infrastructure and development pilots are operational. The
trained-model results below are pipeline and dose pilots, not primary Stage-1
evidence: they use one seed and 25 development answers. No trained checkpoint
has seen the locked 1,000-answer test. The full base-model baseline is the only
unconditional test evaluation the design allows.
...
The most useful causal story so far is:
...
Do not select a representation, present DPO/ORPO/GRPO as a method screen, or
open the trained-model test gate yet. The next development experiment should
test a common-word/action curriculum or a substantially larger SFT dose, while
preserving the frozen evaluator and recording the changed training signal. Begin
the preregistered Stage-2 comparison only after three seed-matched SFT parents
show nonzero strategic performance.

## Follow-up intervention iteration

Date: 2026-08-14

The requested follow-up reproduced the repeated-word collapse and separated four
causes that the original pilots had conflated:
...
The base prompt’s literal `Final answer: WORD` example also explains why the
unfine-tuned model emitted the placeholder `WORD`: that was prompt imitation,
...
Temperature was not the useful lever after training. At temperature 0.7,
diversity rose to 52 words and repeats fell to 26.7%, but the win disappeared and
compliance fell to 98.7%. A repetition penalty of 1.10 cut repeats to 4.8% but
introduced 4.6% invalid calls. The 1.05 penalty retained perfect compliance and
the win.
...
The repeated-word collapse is improved within a bounded development regime: the
best clean condition uses 43 unique guesses with a 10.3% repeat rate, 100%
strict validity, and one held-out win. This is not evidence of broad Wordle
competence: constraint violations remain 82.2%, the development sample is 25
games, and only one seed has run. Do not open the locked protocol-002 test gate.
