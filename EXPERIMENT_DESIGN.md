...
Status: proposed architecture; no training runs have been started.

## 1. Research questions

This project should answer three separate questions:
...
These must not be collapsed into one large leaderboard. Method, data representation,
training dose, and random seed are separate experimental factors.

## 2. Frozen contract

Before running the new study, create a new protocol version (for example,
`WORDLE-PROTOCOL-002`) and a new full baseline. The earlier exact-word protocol is a
historical result and must not be mixed with results from the reasoning-capable
protocol.

The following are evaluation infrastructure, not experimental treatments:
...
The evaluator must never mask logits to the vocabulary, choose a guess for the model,
ban repeated words, inject candidate words, repair malformed output, or truncate a
response into a valid guess. Instrumentation may be added as long as it only observes
the model's behavior.

### Reasoning-capable output protocol

Every condition uses the same terminal-answer envelope:
...
Direct-output datasets use only `Final answer: CRANE`. Reasoning datasets begin the
assistant completion immediately with the reasoning trace and place the same terminal
line at the end. Thus the difference is the learned reasoning behavior, not a special
parser or a different answer syntax.

Use the same generation ceiling for every condition (initial proposal: 128 new
tokens), retain EOS/end-of-turn stopping, and do not stop generation programmatically
when the terminal marker first appears. A direct model may finish early; a reasoning
model may use more of the common budget. If a model omits the terminal line, puts prose
after it, or exhausts the budget before completing it, the call is a format failure.
Record generated-token count so improvements can be compared against inference cost.
...
There are two honest method comparisons:
...
episodes and states. The answer is metadata and is never included in the model-visible
prompt.
...
Visible rationale training may improve decision quality while increasing latency or
format failures. Report all three rather than hiding the tradeoff. A
`rationale_then_direct` curriculum may be run as a diagnostic, but not as a fifth
...
## 5. Staged experiment, not a full Cartesian explosion

Use development answers for every selection decision and open the locked test only
after the selection rules have been applied.
...
Using the best direct-output-compatible representation and seed-matched SFT warm
starts, run continued SFT, DPO, ORPO, and GRPO with three seeds. Use a small,
predeclared hyperparameter grid on development data only; do not give one technique a
large search while leaving the others at defaults.
...
expensive. Report two views instead of hiding the mismatch:
...
For every valid guess, compute without affecting generation:
...
These diagnostics separate three possible causes of improvement: better formatting,
better rule/state tracking, and better strategic word selection.
...
The final report should generate, from tidy result tables rather than hand-entered
values:
...
This is a maximum of 36 primary training runs before reuse, not a blind
...
If the best method changes across seeds, compute budgets, or data representations,
the conclusion should be conditional rather than naming a universal winner.
