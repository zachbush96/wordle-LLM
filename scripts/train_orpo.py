...
def main() -> int:
    parser = argparse.ArgumentParser(description="Standalone matched-data ORPO")
    ...
    parser.add_argument("--lambda-or", type=float, default=0.1); parser.add_argument("--dry-run", action="store_true")
    ...
    spec = {**base_spec("orpo", args.partition, args.seed, args.steps, args.learning_rate), "lambda_or": args.lambda_or, "parent_checkpoint": str(args.parent_adapter)}
    assert_gemma_parent_adapter(args.parent_adapter)
    if args.dry_run: print(json.dumps(dry_run_summary("orpo", args.partition, data_dir, rows, spec, audit=audit), indent=2, sort_keys=True)); return 0
    run_dir = prepare_run("orpo", args.partition, spec, data_dir); write_jsonl(run_dir / "preferences.jsonl", rows); set_seed(args.seed)
    train_orpo(rows, args.parent_adapter, run_dir, spec); summary = {"status": "trained", "run_dir": str(run_dir), "preference_pairs": len(rows)}; write_json(run_dir / "summary.json", summary)
