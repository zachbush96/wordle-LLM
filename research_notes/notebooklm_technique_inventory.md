...
The conversation distinguishes LoRA (a parameter-efficient adapter mechanism) from the training objective. It proposes comparing SFT, DPO, ORPO, GRPO, Q-SFT, and a staged SFT-to-GRPO pipeline. The notebook overview also mentions Advantage Collapse Rate (ACR) and Adaptive Virtual Sample Policy Optimization (AVSPO). Supporting interventions include partial-game-state curricula, structured character-list prompts, a multi-component reward, and a harness-fixed opening guess.
...
The notebook claims a 100-target Gemma 3 270M comparison with win rates of 4.2% SFT, 6.8% DPO, 7.1% ORPO, 11.3% GRPO, 13.8% Q-SFT, and 15.6% hybrid SFT+GRPO. These values are hypotheses/context only. They are not measurements from this repository and must never be merged with local development results as though they shared datasets, prompts, seeds, or evaluation code.
...
The NotebookLM rubric is wired as an optional GRPO training reward under
...
The default historical `wordle-shaped-v1` reward is unchanged, so the new
rubric can be tested as a matched ablation instead of silently changing old runs.
